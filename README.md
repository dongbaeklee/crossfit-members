...
엑셀을 기다리지 않고 바로 넣고 뺄 수 있다. 시드와 부딪히지 않게 두 가지를 구분해 둔다.

| | 어떻게 표시되나 | 주간 시드가 하는 일 |
| --- | --- | --- |
| 화면에서 추가 | `source = 'manual'` | **건드리지 않는다** — 엑셀에 없어도 그대로 남는다 |
| 화면에서 삭제 | `archived_at` 설정 (행은 남음) | **되살리지 않는다** — 엑셀에 아직 있어도 숨은 채로 유지 |

- **삭제는 하드 삭제가 아니다.** 코치가 쌓아둔 역량·특성(`member_profiles`)이 남고, 같은 이름으로 다시 추가하면 그대로 되살아난다.
- 쓰기 권한은 테이블에 열지 않고 **RPC 두 개**(`add_member` / `archive_member`)만 연다. 코치가 엑셀에서 온 회원권·출석 값을 임의로 고칠 수 없게 하기 위함이다.
- 권한은 코치 이상, 자기 지점만(대표는 전체).

보관된 회원을 SQL 로 되살리려면:

```sql
update public.member_snapshots set archived_at = null
where box = '메이커스' and name = '홍길동';
```
