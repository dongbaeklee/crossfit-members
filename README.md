...
툴바의 **엑셀 내려받기** → 지금 화면에 보이는 목록이 `.xlsx` 로 저장된다(지점 필터·검색이 그대로 반영되고, 파일 이름에도 적힌다).

담기는 것: 지점·이름·상태·회원권·시작/종료일·가입일·최근출석·등록경로 + **역도/체조/유산소 점수, 강점, 약점, 목표, 성향, 리스크, 실제 운동 시작일, 최근 수정**.
역량·특성은 이 시스템에만 있는 데이터라 **내려받기가 곧 백업**이다.

> xlsx 생성기는 `src/lib/xlsx.ts` 에 직접 구현했다(의존성 0). SheetJS 를 쓰면 번들이 거의 두 배가 되는데,
> 가끔 쓰는 기능 때문에 매 페이지 로딩이 그 값을 치르게 된다. 실제 증가분은 gzip 기준 +2KB.
> CSV 를 쓰지 않은 이유는 한글이 엑셀에서 깨지는 사고가 흔해서다.
>
> 생성기를 고치면 반드시 `npx vite-node scripts/check-xlsx.ts` 로 파일을 만들어 실제로 열어볼 것.
> zip/XML 을 손으로 쓰는 코드라, 안 열어보면 "엑셀에서 파일이 손상됐다고 나옴" 을 사용자가 먼저 발견하게 된다.
